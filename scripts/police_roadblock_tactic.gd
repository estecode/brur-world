class_name PoliceRoadblockTactic
extends RefCounted

## Models an abstract roadblock tactic lifecycle at a routing-supplied target.
##
## Dependencies:
...
## - Does not encode deployment geometry, timing, police procedure or Vehicle physics.

enum State { IDLE, REQUESTED, PREPARING, ACTIVE, FINISHED, CANCELLED }

const TACTIC_ID: StringName = &"roadblock"
...
var prepare_duration_s: float = 1.5
var active_duration_s: float = 10.0
var abstract_lead_distance_m: float = 130.0
var _routing_owner
var _reservations
...
func request(observation: Dictionary) -> Dictionary:
	...
	if not observed_position.is_finite():
		return {}
	var candidate_value: Variant = _routing_owner.call("advance_along_road", observed_position, float(observation.get("heading_deg", 0.0)), abstract_lead_distance_m)
...
func step(delta: float, unit_position: Vector2) -> void:
	...
	if state == State.REQUESTED:
		...
		if unit_position.is_finite() and target_position.is_finite() and unit_position.distance_to(target_position) <= 10.0 and prepare_elapsed_s >= prepare_duration_s:
			state = State.ACTIVE
...
func should_replan(observation: Dictionary) -> bool:
	if state == State.IDLE or is_terminal() or observation.is_empty():
		return false
	if bool(observation.get("stale", false)) or not bool(observation.get("current", false)):
		return true
	var timestamp_s := float(observation.get("timestamp_s", source_timestamp_s))
	if timestamp_s <= source_timestamp_s:
		return false
	var observed_position: Vector2 = observation.get("position", Vector2(INF, INF))
	return observed_position.is_finite() and target_position.is_finite() and observed_position.distance_to(target_position) > 45.0
...
func snapshot() -> Dictionary:
	return {"tactic_id": TACTIC_ID, "unit_id": unit_id, "state": state, "state_name": str(State.keys()[state]), "target_position": target_position, "source_timestamp_s": source_timestamp_s}
