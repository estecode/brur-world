...
func _update_distance_policy() -> void:
	...
	var target_lod := 1 if distance < FAR_PRESENTATION_M else 0
	_hlod.set_desired_lod("world",target_lod)
	if want_detail:
		_hlod.request("world",1)
		var s: Dictionary=geodot_world_layer.call("debug_snapshot")
		var desired:=int(s.get("desired_cells",0)); var ready:=int(s.get("ready_desired_cells",s.get("active_cells",0)))
		if desired>0 and ready>=desired: _hlod.mark_ready("world",1)
	_apply_hlod_ownership()

func _apply_hlod_ownership() -> void:
	var detail_owner:=_hlod.visible_lod("world")==1
	if geodot_world_layer!=null and geodot_world_layer.has_method("set_presentation_visible"): geodot_world_layer.call("set_presentation_visible",detail_owner)
...
func geodot_debug_snapshot() -> Dictionary:
	var snapshot:=super.geodot_debug_snapshot(); snapshot["hlod"]=_hlod.snapshot(); snapshot["hlod"]["visible_lod"]=_hlod.visible_lod("world"); snapshot["hlod"]["base_ready"]=_base_ready
	snapshot["transition"]={"detail_owner":_hlod.visible_lod("world")==1,"effective_resident_cells":_effective_resident_cells,"tall_3d_distance_m":float(_tuning.get("tall_3d_distance_m",6000.0))}; return snapshot
