...
const MAX_CLOUDS_PER_CELL: int = 10
const DEFAULT_SEED: int = 700031
const SMALL_PROFILE_WEIGHT: float = 0.76
...
const LARGE_PROFILE := {
	...
	"size_min_m": 4200.0,
	"size_max_m": 12000.0,
	"thickness_min_m": 900.0,
	"thickness_max_m": 2500.0,
	"altitude_min_m_asl": 2200.0,
	"altitude_max_m_asl": 4800.0,
	"speed_min_mps": 10.0,
	"speed_max_mps": 18.0,
...
func generate_cell(cell: Vector2i, coverage: float, seed: int = DEFAULT_SEED) -> Array[Dictionary]:
	...
	var cluster_count: int = clampi(int(ceil(float(cloud_count) / 4.0)), 1, 3)
	var cluster_centers: Array[Vector2] = []
	for _cluster_index in range(cluster_count):
		cluster_centers.append(Vector2(
			rng.randf_range(0.12, 0.88) * CELL_SIZE_M,
			rng.randf_range(0.12, 0.88) * CELL_SIZE_M
		))

	var result: Array[Dictionary] = []
	for cloud_index in range(cloud_count):
		var profile: Dictionary = SMALL_PROFILE if rng.randf() < SMALL_PROFILE_WEIGHT else LARGE_PROFILE
		var center: Vector2 = cluster_centers[cloud_index % cluster_centers.size()]
		var angle: float = rng.randf_range(0.0, TAU)
		var radius: float = sqrt(rng.randf()) * CELL_SIZE_M * 0.20
