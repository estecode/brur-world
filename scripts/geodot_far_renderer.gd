...
var _coordinates=null; var _camera_rig:Node=null; var _cache=FarCache.new(); var _instance:MultiMeshInstance3D=null
var _enabled:=false; var _active:=false; var _refresh_accum:=0.0; var _last_samples:=0; var _last_level:=-1; var _last_cell_m:=0.0; var _last_mpp:=0.0; var _last_build_ms:=0.0; var _last_pressure:=0.0

func setup(world_coordinates,camera_rig:Node,cache_path:String)->Dictionary:
	_coordinates=world_coordinates; _camera_rig=camera_rig; var opened:=_cache.open(cache_path)
	if opened.get("ok",false)!=true:return opened
	_instance=MultiMeshInstance3D.new(); _instance.name="GeoDotFarAggregate"; add_child(_instance); _enabled=true; set_process(true); return opened
func shutdown()->void:
...
func _refresh()->void:
	...
	var focus:Vector3=_camera_rig.call("get_focus_world"); var camera:=get_viewport().get_camera_3d() if get_viewport()!=null else null
	if camera==null:return
	var distance:=camera.global_position.distance_to(focus); _active=distance>=exit_distance_m if _active else distance>=enter_distance_m; visible=_active
	...
	_last_level=_cache.choose_level(maxf(2000.0,_last_mpp*target_screen_cell_px))
	if _last_level<0:return
	_last_pressure=_renderer_memory_pressure(); var budget:=LodPolicy.sample_budget(base_sample_budget,density_scale*LodPolicy.quality_scale_for_pressure(_last_pressure),hard_sample_cap)
	var started:=Time.get_ticks_usec(); var samples:=_cache.query_bounds(bounds,_last_level,budget,density_scale); _publish(samples); _last_samples=samples.size(); _last_build_ms=float(Time.get_ticks_usec()-started)/1000.0
...
func _publish(samples:Array[Dictionary])->void:
	...
	var material:=StandardMaterial3D.new(); material.shading_mode=BaseMaterial3D.SHADING_MODE_UNSHADED; material.albedo_color=Color(0.32,0.33,0.34,0.72); material.transparency=BaseMaterial3D.TRANSPARENCY_ALPHA; material.cull_mode=BaseMaterial3D.CULL_DISABLED; quad.material=material
	var mm:=MultiMesh.new(); mm.transform_format=MultiMesh.TRANSFORM_3D; mm.instance_count=samples.size(); mm.mesh=quad
	for index in range(samples.size()):
		var sample:Dictionary=samples[index]; var cell_m:=float(sample.cell_m); var coverage:=clampf(float(sample.coverage)*density_scale,0.0,1.0); var center_abs:=Vector2((float(sample.x)+0.5)*cell_m,(float(sample.y)+0.5)*cell_m); var center_world:Vector3=_coordinates.absolute_to_world(center_abs); var side:=cell_m*clampf(sqrt(coverage),0.08,0.95)
...
func debug_snapshot()->Dictionary:
	return {"active":_active,"samples":_last_samples,"level":_last_level,"cell_m":_last_cell_m,"meters_per_pixel":_last_mpp,"build_ms":_last_build_ms,"density":density_scale,"sample_budget":base_sample_budget,"ram_pressure":_last_pressure,"ram_target_mb":ram_target_bytes/(1024*1024),"ram_hard_mb":ram_hard_bytes/(1024*1024)}
