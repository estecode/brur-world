...
var _active:Dictionary={}; var _desired:Dictionary={}; var _queue:Array[Dictionary]=[]; var _queued:Dictionary={}; var _ready_results:Array[Dictionary]=[]; var _workers:Array[Dictionary]=[]; var _generation:=0; var _far_screen_lod:=false
var _building_material:StandardMaterial3D=null; var _road_material:StandardMaterial3D=null
var _perf_query_ms:=0.0; var _perf_query_max_ms:=0.0; var _perf_build_ms:=0.0; var _perf_build_max_ms:=0.0; var _perf_queries:=0; var _perf_publishes:=0; var _perf_building_features:=0; var _perf_road_features:=0
...
func shutdown()->void:
	_enabled=false;_ready=false;set_process(false);_generation+=1;_queue.clear();_queued.clear();_ready_results.clear();_desired.clear();_shutdown_query_workers();_clear_active(true);_release_render_resources();_release_source();_coordinates=null;_camera_rig=null
...
static func coverage_cell_size_for_bounds(bounds:Rect2,base_cell_size:float,margin_cells:int,max_cells:int)->float:return StreamingPolicy.bounded_cell_size_for_bounds(bounds,base_cell_size,margin_cells,max_cells)
...
func _view_world_points(focus:Vector3)->Array[Vector3]:
	...
	if _camera_rig.has_method("get_ground_view_corners"):
		var corners:Variant=_camera_rig.call("get_ground_view_corners");if typeof(corners)==TYPE_PACKED_VECTOR3_ARRAY or typeof(corners)==TYPE_ARRAY:
			for v in corners:if typeof(v)==TYPE_VECTOR3 and (v as Vector3).is_finite():p.append(v)
	if p.is_empty():p.append(focus);return p
...
func _drop_obsolete_ready_results()->void:
	var kept:Array[Dictionary]=[];for result in _ready_results:var req:Dictionary=result.get("request",{});var key:=String(req.get("key",""));if int(req.get("generation",-1))==_generation and _desired.has(key) and int(_desired[key])==int(req.get("lod",-1)):kept.append(result);_ready_results=kept
...
func _publish_cell(request:Dictionary,result:Dictionary)->void:
	...
	add_child(group);_active[key]={"node":group,"lod":lod,"origin_abs":origin,"cell_size_m":size,"buildings":int(result.get("building_features",0))};group.visible=_presentation_visible
...
func ready_coverage_rects()->Array[Rect2]:
	...
	return out
func debug_snapshot()->Dictionary:return {"enabled":_enabled,"ready":_ready,"presentation_visible":_presentation_visible,"active_cells":_active.size(),"desired_cells":_desired.size(),"ready_desired_cells":ready_coverage_rects().size(),"pending_cells":_queue.size()+_workers.size(),"ready_cells":_ready_results.size(),"screen_lod":"far" if _far_screen_lod else "near","max_resident_cells":max_resident_cells,"source":source_metadata()}
func consume_perf_metrics()->Dictionary:return {"renderer":"geodot","geodot_active_cells":_active.size(),"geodot_pending_cells":_queue.size()+_workers.size()}
