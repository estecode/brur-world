...
func target_speed_mps(
	...
	var target := _speed_limit_at(index, speed_limits_mps) * profile.speed_limit_multiplier
	...
	for i in range(index, mini(points.size() - 1, index + 12)):
		...
		var curve_speed := maxf(MIN_CURVE_SPEED_MPS, sqrt(profile.max_lateral_accel_mps2 * radius_m))
		target = minf(target, _approach_speed(curve_speed, distance_ahead, profile.comfort_brake_mps2))
		var upcoming_limit := _speed_limit_at(i + 1, speed_limits_mps) * profile.speed_limit_multiplier
