...
func open(path: String) -> Dictionary:
	close()
	if path.is_empty() or not FileAccess.file_exists(path): return {"ok": false, "error": "far cache missing"}
	var file := FileAccess.open(path, FileAccess.READ)
	if file == null: return {"ok": false, "error": "far cache open failed"}
	var parsed: Variant = JSON.parse_string(file.get_as_text())
	if typeof(parsed) != TYPE_DICTIONARY: return {"ok": false, "error": "far cache JSON invalid"}
	var payload: Dictionary = parsed
	if int(payload.get("schema", -1)) != SCHEMA: return {"ok": false, "error": "far cache schema mismatch"}
	_source = payload.get("source", {})
	for level_value in payload.get("levels", []):
		if typeof(level_value) != TYPE_DICTIONARY: continue
		var level: Dictionary = level_value; var index: Dictionary = {}
...
func level_cell_size(level_index:int) -> float:
	if level_index < 0 or level_index >= _levels.size(): return 1.0
	return maxf(1.0,float(_levels[level_index].cell_m))
...
func query_bounds(bounds: Rect2, level_index: int, max_samples: int, density_scale: float = 1.0) -> Array[Dictionary]:
	...
	var min_x := floori(bounds.position.x / cell_m); var min_y := floori(bounds.position.y / cell_m); var end := bounds.position + bounds.size
	var max_x := floori(end.x / cell_m); var max_y := floori(end.y / cell_m); var candidates: Array[Dictionary] = []
	for y in range(min_y, max_y + 1):
		for x in range(min_x, max_x + 1):
			var key := "%d:%d" % [x,y]
			if not index.has(key): continue
			var cell: Array = index[key]; var coverage := clampf(float(cell[3]) / maxf(1.0, cell_m * cell_m), 0.0, 1.0); var weight := coverage * maxf(0.0, density_scale)
