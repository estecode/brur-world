...
class BuildingPocPrepareTests(unittest.TestCase):
    def test_prepare_filters_existing_buildings_into_tiles(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            tile_files = list(output.glob("*.jsonl"))
            self.assertEqual(len(tile_files), 1)
            cached = json.loads(tile_files[0].read_text(encoding="utf-8").strip())
            self.assertEqual(cached["x"], inside["x"])
            self.assertEqual(cached["y"], inside["y"])
