...
func _test_building_identity_and_mesh() -> void:
	...
	if mesh == null:
		return
	_assert(mesh.get_surface_count() == 1, "offline/reference building mesh builder emits one batch")
	var bounds := mesh.get_aabb()
	_assert(bounds.size.x > 9.9 and bounds.size.z > 9.9, "building footprint keeps metre-scale horizontal bounds")
	_assert(bounds.size.y >= 11.9 and bounds.size.y <= 12.1, "building height is preserved in reference mesh bounds")
...
func _test_atomic_viewport_staging_and_cache() -> void:
	...
	await _wait_until_ready(layer, 120)
	var latest_snapshot: Dictionary = layer.debug_snapshot()
	_assert(String(latest_snapshot["active_signature"]) == String(latest_snapshot["desired_signature"]), "stale staging cannot publish over the latest camera request")
