...
func _init() -> void:
	...
	var player := player_scene.instantiate() as Vehicle
	_assert(player != null, "player vehicle scene instantiates Vehicle adapter")

	var root := Node.new()
	get_root().add_child(root)
	root.add_child(player)
	player.set_world_position(Vector3(100.0, 0.0, 200.0))

	var player_controller := player.get_node_or_null("PlayerVehicleController")
	var route_follower := player.get_node_or_null("VehicleRouteFollower")
	...
	_assert(player.control_owner() == Vehicle.ControlOwner.PLAYER, "player owns controls initially")

	_assert(player.set_control_inputs(Vehicle.ControlOwner.PLAYER, 1.0, 0.0, 0.0), "active owner control is accepted")
	_assert(not player.set_control_inputs(Vehicle.ControlOwner.GPS, 1.0, 0.0, 0.0), "inactive owner control is rejected")
	player._physics_process(1.0)
	var after_manual = player.state_snapshot()
	_assert(after_manual.speed_mps > 0.0, "adapter advances through production dynamics")

	var before_switch = player.state_snapshot()
	player.set_control_owner(Vehicle.ControlOwner.GPS)
	var after_switch = player.state_snapshot()
	...
	_assert(player.control_owner() == Vehicle.ControlOwner.GPS, "GPS owns controls while follow is on")
	route_follower.call("_physics_process", 1.0 / 60.0)
	player._physics_process(1.0 / 60.0)
	var before_manual_takeover = player.state_snapshot()
	_assert(bool(route_follower.call("set_follow_enabled", false)), "follow can be disabled")
	var after_manual_takeover = player.state_snapshot()
	_assert(player.control_owner() == Vehicle.ControlOwner.PLAYER, "manual control owns same vehicle after follow off")
