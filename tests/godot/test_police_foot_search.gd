...
func _test_direct_observation_and_lost_contact() -> void:
	...
	_assert((direct.get("target_position", Vector2.INF) as Vector2).is_equal_approx(Vector2(10, 0)), "direct pursuit targets observed position")
	...
	var frozen := ai.last_known.snapshot(2.0)
	_assert((frozen.get("position", Vector2.INF) as Vector2).is_equal_approx(Vector2(10, 0)), "last-known position remains frozen after sight loss")
...
func _test_search_area_growth_obstacles_and_roads() -> void:
	...
	var local_space = FakeLocalSpace.new(); local_space.block_positive_x = true
...
func _test_reacquisition_and_low_confidence() -> void:
	...
	var ai = FootAIScript.new(); ai.setup(store)
	...
	_assert((reacquired.get("target_position", Vector2.INF) as Vector2).is_equal_approx(Vector2(20, 0)), "reacquisition uses new observed position")
	var low_store = ObservationStoreScript.new(10.0)
	_publish(low_store, Vector2(7, 0), 1.0, false)
	var low_ai = FootAIScript.new(); low_ai.setup(low_store)
...
func _test_multiple_officers_and_transport_secrecy() -> void:
	...
	var first = FootAIScript.new(); first.setup(store)
	var second = FootAIScript.new(); second.setup(store)
	...
	_assert((first.last_known.snapshot(4.0).get("position", Vector2.INF) as Vector2).is_equal_approx(Vector2(12, 4)), "unobserved transport transition does not publish a new exact position")
	_assert((after_transport.get("target_position", Vector2.INF) as Vector2).distance_to(unobserved_transport_position) > 400.0, "search remains based on last observed state after hidden transport")
...
func _test_generic_person_controller_moves_from_observation_only() -> void:
	...
	_publish(store, Vector2(10, 0), 0.0)
	var ai = FootAIScript.new(); ai.setup(store)
	var controller = PersonControllerScript.new(); root.add_child(controller)
	...
	controller.queue_free(); person.queue_free()
