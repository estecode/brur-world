...
func _test_stale_cells_retire_when_coverage_shrinks() -> void:
	...
	var keep := Node3D.new()
	var stale_a := Node3D.new()
	var stale_b := Node3D.new()
	renderer.add_child(keep)
	renderer.add_child(stale_a)
	renderer.add_child(stale_b)
	renderer._active = {
		"0:0": {"node": keep, "lod": 1},
		"1:0": {"node": stale_a, "lod": 1},
		"2:0": {"node": stale_b, "lod": 1},
	}
