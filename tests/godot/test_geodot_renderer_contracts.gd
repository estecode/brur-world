...
func _run() -> void:
	_test_building_adapter(); _test_geographic_source_conversion(); _test_real_osm_feature_filtering(); _test_cell_ownership_is_unique(); _test_road_adapter(); _test_road_cell_clipping_is_seam_safe(); _test_full_road_strip_clipping_is_seam_safe(); _test_stale_cell_eviction_prefers_replaced_outer_cell(); _test_stale_cells_remain_warm_when_coverage_shrinks(); _test_far_building_lod_preserves_feature(); _test_road_mesh_batching()
	if _failed: quit(1); return
	print("geodot renderer contracts: OK"); quit(0)

func _test_building_adapter() -> void:
	var feature := FakePolygonFeature.new(); feature.feature_id = 42
	feature.outer = PackedVector2Array([Vector2(1000.0,2000.0),Vector2(1020.0,2000.0),Vector2(1020.0,2010.0),Vector2(1000.0,2010.0)])
	feature.holes = [PackedVector2Array([Vector2(1005.0,2003.0),Vector2(1010.0,2003.0),Vector2(1010.0,2007.0),Vector2(1005.0,2007.0)])]
...
func _test_geographic_source_conversion() -> void:
	var lund_lonlat := Vector2(13.1910,55.7047); var absolute := GeoDotWorldSourceScript.source_to_absolute(lund_lonlat,4326)
	var expected_x := 6378137.0 * deg_to_rad(lund_lonlat.x); var lat_rad := deg_to_rad(lund_lonlat.y); var expected_y := 6378137.0 * log(tan(PI*0.25+lat_rad*0.5))
...
func _test_real_osm_feature_filtering() -> void:
	...
	_assert(not road.is_empty(), "highway promoted only through GDAL other_tags is retained"); _assert(int(road.get("road_class",-1))==5,"residential other_tags road maps to local-road class")

func _test_cell_ownership_is_unique() -> void:
	var left_min:=Vector2(0,0); var left_max:=Vector2(10,10); var right_min:=Vector2(10,0); var right_max:=Vector2(20,10); var boundary_record={"x":10.0,"y":5.0}
	...
	var interior_record={"x":9.5,"y":5.0}; _assert(GeoDotWorldSourceScript.record_owned_by_cell(interior_record,left_min,left_max),"interior building remains owned by its source cell"); _assert(not GeoDotWorldSourceScript.record_owned_by_cell(interior_record,right_min,right_max),"interior building cannot be duplicated into an adjacent cell")
...
func _test_road_cell_clipping_is_seam_safe() -> void:
	var a:=Vector2(0,5); var b:=Vector2(20,5); var left:=GeoDotWorldMeshBuilderScript.clip_segment_to_cell(a,b,Vector2(0,0),Vector2(10,10)); var right:=GeoDotWorldMeshBuilderScript.clip_segment_to_cell(a,b,Vector2(10,0),Vector2(20,10))
	...
	if left.size()==2 and right.size()==2:
		_assert(left[1].x<10.0,"left cell uses a half-open max edge so the road is not duplicated on the boundary"); _assert(is_equal_approx(right[0].x,10.0),"right cell owns the shared boundary from its inclusive min edge"); _assert(right[0].x-left[1].x<=0.0011,"cell clipping leaves no visually meaningful road gap")
	var outside:=GeoDotWorldMeshBuilderScript.clip_segment_to_cell(Vector2(0,20),Vector2(20,20),Vector2(0,0),Vector2(10,10)); _assert(outside.is_empty(),"road segment outside a cell emits no duplicate geometry")

func _test_full_road_strip_clipping_is_seam_safe() -> void:
	var segment_a:=Vector2(2,2); var segment_b:=Vector2(18,18); var delta:=segment_b-segment_a; var side:=Vector2(-delta.y,delta.x).normalized()*4.0
	var full_strip:=PackedVector2Array([segment_a-side,segment_a+side,segment_b+side,segment_b-side]); var left:=GeoDotWorldMeshBuilderScript.clip_polygon_to_cell(full_strip,Vector2(0,0),Vector2(10,20)); var right:=GeoDotWorldMeshBuilderScript.clip_polygon_to_cell(full_strip,Vector2(10,0),Vector2(20,20))
	_assert(left.size()>=3 and right.size()>=3,"diagonal road strip crossing a cell edge survives on both sides"); var left_max_x:=-INF; var right_min_x:=INF
	for point in left: left_max_x=maxf(left_max_x,point.x); _assert(point.x>=-0.0001 and point.x<=10.0,"left road strip is fully bounded by its cell")
	for point in right: right_min_x=minf(right_min_x,point.x); _assert(point.x>=9.999 and point.x<=20.0001,"right road strip is fully bounded by its cell")
	_assert(right_min_x-left_max_x<=0.0011,"adjacent clipped road strips leave no visible seam")
	var outside:=GeoDotWorldMeshBuilderScript.clip_polygon_to_cell(PackedVector2Array([Vector2(30,30),Vector2(31,30),Vector2(31,31),Vector2(30,31)]),Vector2(0,0),Vector2(10,10)); _assert(outside.is_empty(),"road strip wholly outside a cell emits no cell geometry")

func _test_stale_cell_eviction_prefers_replaced_outer_cell() -> void:
	var active={"0:0":{},"1:0":{},"2:0":{}}; var desired={"1:0":1,"2:0":1,"3:0":1}; var stale:=GeoDotWorldRendererScript.choose_stale_eviction_key(active,desired,"3:0")
	_assert(stale=="0:0","publishing a new desired cell evicts a stale outer cell rather than a still-desired cell"); var no_stale:=GeoDotWorldRendererScript.choose_stale_eviction_key(active,{"0:0":1,"1:0":1,"2:0":1},"3:0"); _assert(no_stale.is_empty(),"resident bound never sacrifices a desired cell when no stale slot exists")

func _test_stale_cells_remain_warm_when_coverage_shrinks() -> void:
	var renderer:=GeoDotWorldRendererScript.new(); var keep:=Node3D.new(); var stale_a:=Node3D.new(); var stale_b:=Node3D.new(); renderer.add_child(keep); renderer.add_child(stale_a); renderer.add_child(stale_b)
	renderer._active={"0:0":{"node":keep,"lod":1},"1:0":{"node":stale_a,"lod":1},"2:0":{"node":stale_b,"lod":1}}; renderer._desired={"0:0":1}; renderer._retire_stale_active_cells()
	_assert(renderer._active.size()==3,"coverage shrink keeps completed detail warm for reverse zoom"); _assert(renderer._stale_active_count()==2,"warm non-owner cells remain explicitly tracked for bounded eviction"); renderer.free()
...
func _test_far_building_lod_preserves_feature() -> void:
	...
	var mesh:=GeoDotWorldMeshBuilderScript.build_buildings([record],Vector2.ZERO,GeoDotWorldMeshBuilderScript.LOD_FAR); _assert(mesh!=null,"far LOD keeps the building instead of culling it")
	if mesh!=null:
		var aabb:=mesh.get_aabb(); _assert(aabb.size.x>=19.9 and aabb.size.z>=15.9,"far building proxy preserves massing extent"); _assert(aabb.size.y>=8.9,"far building proxy preserves semantic height")
...
func _test_road_mesh_batching() -> void:
	...
	var mesh:=GeoDotWorldMeshBuilderScript.build_roads(roads,Vector2.ZERO,GeoDotWorldMeshBuilderScript.LOD_NEAR); _assert(mesh!=null,"multiple GeoDot roads batch into one mesh")
