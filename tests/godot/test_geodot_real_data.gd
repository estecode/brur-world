...
func _run() -> void:
	...
	if gpkg.is_empty(): quit(1); return
	...
	if not FileAccess.file_exists(manifest_path): quit(1); return
	...
	var cell := Vector2i(floori(lund_abs.x / CELL_SIZE), floori(lund_abs.y / CELL_SIZE)); var top_left := Vector2(float(cell.x) * CELL_SIZE, float(cell.y + 1) * CELL_SIZE)
	var source = SourceScript.new(); var opened: Dictionary = source.open_dataset(gpkg)
	_assert(opened.get("ok", false) == true, "GeoDot opens production GeoPackage and resolves road/building layers")
	if opened.get("ok", false) != true: print("GeoDot open result: ", opened); source.close(); quit(1); return
	...
	_assert(result.get("ok", false) == true, "Lund cell query succeeds"); _assert(int(result.get("building_features", 0)) > 0, "Lund query contains buildings"); _assert(int(result.get("road_features", 0)) > 0, "Lund query contains roads")
	_assert(int(result.get("raw_building_features", 0)) < MAX_BUILDING_SOURCE_FEATURES, "Lund mixed polygon source query does not saturate before building filtering"); _assert(int(result.get("raw_road_features", 0)) < MAX_ROAD_SOURCE_FEATURES, "Lund mixed line source query does not saturate before highway filtering")
	var build_started := Time.get_ticks_usec(); var building_mesh := MeshBuilderScript.build_buildings(result.get("buildings", []), Vector2(float(cell.x) * CELL_SIZE, float(cell.y) * CELL_SIZE), MeshBuilderScript.LOD_FAR); var road_mesh := MeshBuilderScript.build_roads(result.get("roads", []), Vector2(float(cell.x) * CELL_SIZE, float(cell.y) * CELL_SIZE), MeshBuilderScript.LOD_FAR); var build_ms := float(Time.get_ticks_usec() - build_started) / 1000.0
	...
	if _failed: quit(1); return
	print("geodot real-data test: OK"); quit(0)

func _run_production_ab_performance() -> void:
	print("GEODOT_AB_PERF starting comparable production Drive measurements")
	if not _run_child_test("res://tests/godot/test_production_fps_real_data.gd", "production Drive FPS real-data test: OK", "legacy"): return
...
func _run_child_test(script: String, marker: String, label: String) -> bool:
	var output: Array = []; var args := PackedStringArray(["--headless", "--path", ProjectSettings.globalize_path("res://"), "--script", script]); var exit_code := OS.execute(OS.get_executable_path(), args, output, true); var combined := ""
	for line in output:
