...
func _test_renderer_structure(model) -> void:
	...
	_assert(int(stats["max_puff_instances"]) == 7200, "cohesive cloudlets keep one explicit MultiMesh budget")
	...
	_assert(float(stats["max_horizontal_aspect"]) > 1.25, "renderer creates irregular horizontal silhouettes")
	_assert(float(stats["max_horizontal_aspect"]) <= 2.10 + 0.01, "horizontal lobe elongation stays bounded")
	_assert(int(stats["cloudlet_count"]) >= int(stats["cloud_count"]), "every formation contains at least one cohesive cloudlet")
	_assert(int(stats["cloudlet_count"]) > int(stats["cloud_count"]), "giant formations split into multiple cloudlets instead of giant single lobes")
	_assert(int(stats["clustered_satellite_count"]) > int(stats["cloud_count"]), "cloudlets use overlapping satellite puffs instead of isolated balls")
