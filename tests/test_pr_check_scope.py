...
def main() -> None:
    ...
    assert world_showcase_real_data_required(("harness/world_showcase/world_showcase.gd",))
    assert not world_showcase_real_data_required(("scripts/building_stream_layer.gd",)), (
        "production building changes are covered by production tests, not showcase preparation"
    )
