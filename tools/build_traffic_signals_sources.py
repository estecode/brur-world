...
def _log(message: str) -> None: print(f"[{_now()}] [traffic-signals] {message}", flush=True)


def build_traffic_signals_sources(signal_source: Path, highway_source: Path, output: Path) -> dict:
    validate_facts(signal_source, SIGNAL_SCHEMA); validate_facts(highway_source, HIGHWAY_SCHEMA)
    output.mkdir(parents=True, exist_ok=True); destination = output / "traffic_signals.json"; started = time.perf_counter()
    ...
    for fact in iter_facts(signal_source, SIGNAL_SCHEMA):
        signal_id = int(fact["osm_id"]); signal_ids.add(signal_id); signals.append(fact)

    memberships: dict[int, list[int]] = {signal_id: [] for signal_id in signal_ids}
    highway_way_count = 0
    for index, fact in enumerate(iter_facts(highway_source, HIGHWAY_SCHEMA), 1):
        highway_way_count += 1
        way_id = int(fact["osm_id"])
        for node_id in fact.get("node_ids", []):
            value = int(node_id)
            if value in memberships: memberships[value].append(way_id)
        if index % 250_000 == 0:
    ...
    for signal in signals:
        ...
        drafts.append(SignalDraft(signal_id, x, y, {str(k):str(v) for k,v in signal.get("tags",{}).items()}, tuple(sorted(set(memberships.get(signal_id, []))))))

    dataset = build_runtime_dataset(drafts); save_runtime_dataset(destination, dataset)
    ...
    manifest["traffic_signals"] = {
        "format": dataset["format"], "file": destination.name,
        "source_cache_blocks": [signal_source.name, highway_source.name],
        "source_signal_count": stats["source_signal_count"],
        "exported_signal_count": stats["exported_signal_count"],
    }
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    _log(f"DONE source={stats['source_signal_count']:,} exported={stats['exported_signal_count']:,} bytes={stats['output_bytes']:,} elapsed={stats['build_seconds']:.2f}s")
    return stats
