...
if [[ "$GEODOT_POC_SCOPE" == "skip" ]]; then
  printf 'PR_CHECK=SKIP_GEODOT_POC pr=%s reason=unrelated-changes\n' "$BRUR_PR_CHECK_PR"
fi
...
run_godot_test() {
  ...
  status=${PIPESTATUS[0]}
  set -e
  if [[ $status -ne 0 ]] || grep -Eq 'SCRIPT ERROR:|Failed to load script|test failed:|ASSERT FAILED:' "$log"; then
