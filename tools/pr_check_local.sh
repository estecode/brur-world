...
ensure_routing_dataset_identity() {
  if "$PYTHON" "$WORKTREE/tools/check_routing_dataset.py" "$WORLD_DATA"; then printf 'PR_CHECK=REUSE_ROUTING_DATASET pr=%s reason=identity-match\n' "$BRUR_PR_CHECK_PR"; return 0; fi
  local pbf; pbf="$(resolve_sweden_pbf)"
...
if [[ "$GEODOT_POC_SCOPE" == "required" ]]; then
  GEODOT_GPKG="$(resolve_geodot_gpkg)"
  printf 'PR_CHECK=GEODOT_GPKG path=%s\n' "$GEODOT_GPKG"
  bash "$WORKTREE/tools/setup_geodot_poc.sh"
  "$PYTHON" "$WORKTREE/tools/check_geodot_gpkg.py" "$GEODOT_GPKG"
  printf 'PR_CHECK=CHECK_GEODOT_CONTRACTS pr=%s\n' "$BRUR_PR_CHECK_PR"
  BRUR_GEODOT_GPKG="$GEODOT_GPKG" run_godot_test res://tests/godot/test_geodot_renderer_contracts.gd 'geodot renderer contracts: OK'
  printf 'PR_CHECK=CHECK_GEODOT_REAL_DATA pr=%s\n' "$BRUR_PR_CHECK_PR"
  BRUR_GEODOT_GPKG="$GEODOT_GPKG" run_godot_test res://tests/godot/test_geodot_real_data.gd 'geodot real-data test: OK'
