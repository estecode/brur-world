#!/usr/bin/env python3
"""Build BRUR normalized source caches from Geofabrik GeoPackage + tiny OSM supplement.

The adapter has two explicit phases:

1. source extraction: SQLite copies requested Geofabrik layers into per-domain
   source files without decoding rows; pyrosm performs one whole-Sweden query
   for the OSM-only supplement (addresses and/or coastline) and writes it to
   disk;
2. normalization: only after every requested source file exists, BRUR decodes
   those files into the existing portable highway/area/fact cache contracts.

The production runtime never reads GeoPackage or OSM provider schemas.
"""
...
DOMAIN_TABLES: dict[str, tuple[str, ...]] = {
    ...
    "pois": (
        "gis_osm_pois_free", "gis_osm_pois_a_free",
        "gis_osm_places_free", "gis_osm_places_a_free",
        "gis_osm_pofw_free", "gis_osm_pofw_a_free",
        "gis_osm_transport_free", "gis_osm_transport_a_free",
        "gis_osm_traffic_free", "gis_osm_traffic_a_free",
    ),
    "areas": (
        "gis_osm_buildings_a_free", "gis_osm_landuse_a_free",
        "gis_osm_natural_a_free", "gis_osm_water_a_free",
...
POI_RULES: dict[str, tuple[str, str]] = {
    ...
    "doctors": ("amenity", "doctors"), "pharmacy": ("amenity", "pharmacy"),
    "fuel": ("amenity", "fuel"), "charging_station": ("amenity", "charging_station"),
    "parking": ("amenity", "parking"), "bank": ("amenity", "bank"), "atm": ("amenity", "atm"),
    ...
    "mall": ("shop", "mall"), "department_store": ("shop", "department_store"),
    "car_repair": ("shop", "car_repair"), "bicycle_shop": ("shop", "bicycle"),
    "hotel": ("tourism", "hotel"), "motel": ("tourism", "motel"), "hostel": ("tourism", "hostel"),
    "attraction": ("tourism", "attraction"), "museum": ("tourism", "museum"),
...
def _route_metadata(path: Path, route: str) -> tuple[dict, int]:
    if route == "areas":
        return area_source_cache_metadata(path), BAF_FOOTER.size
    if route == "highways":
...
def _route_valid(cache_dir: Path, route: str, entry: object, source_identity: dict) -> bool:
    ...
    if not path.is_file() or entry.get("size_bytes") != path.stat().st_size:
        return False
    try:
        meta, _ = _route_metadata(path, route)
    except (OSError, ValueError, TypeError, KeyError):
        return False
    return str(meta.get("sha256", "")) == entry.get("sha256")
...
def _stage_domain(gpkg: Path, destination: Path, domain: str, tables: tuple[str, ...]) -> dict:
    ...
    try:
        available = [table for table in tables if _table_exists(source, table)]
    finally:
        source.close()
    if not available:
        raise ValueError(f"GeoPackage has no required {domain} layers: {', '.join(tables)}")
...
def _stage_required_domains(routes: Iterable[str]) -> tuple[str, ...]:
    ...
    if route_set & {"highways", "traffic_signals"}: selected.append("roads")
    if "traffic_signals" in route_set: selected.append("traffic")
    if "pois" in route_set: selected.append("pois")
    if "areas" in route_set: selected.append("areas")
    return tuple(selected)


def stage_geopackage(gpkg: Path, stage_dir: Path, routes: Iterable[str], gpkg_identity: dict) -> dict[str, Path]:
    stage_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = stage_dir / "geofabrik_manifest.json"
    old = _load_json(manifest_path)
    old_domains = old.get("domains", {}) if old.get("source") == gpkg_identity and isinstance(old.get("domains"), dict) else {}
    domains: dict[str, dict] = dict(old_domains)
    outputs: dict[str, Path] = {}
    for domain in _stage_required_domains(routes):
        path = stage_dir / f"{domain}.sqlite"
        entry = old_domains.get(domain)
        valid = isinstance(entry, dict) and entry.get("complete") is True and entry.get("size_bytes") == (path.stat().st_size if path.is_file() else -1)
        if valid:
            _log(f"STAGE HIT domain={domain} bytes={path.stat().st_size:,}")
        else:
            report = _stage_domain(gpkg, path, domain, DOMAIN_TABLES[domain])
            domains[domain] = {"complete": True, **report}
            _atomic_json(manifest_path, {"format": STAGE_FORMAT, "version": STAGE_VERSION, "source": gpkg_identity, "domains": domains})
...
def stage_pyrosm_supplement(pbf: Path, destination: Path, *, addresses: bool, coastline: bool, source_identity: dict) -> Path:
    if not addresses and not coastline:
        raise ValueError("supplement requested without any predicates")
    ...
    if destination.is_file() and old.get("complete") is True and old.get("source") == source_identity and old.get("predicates") == predicates and old.get("size_bytes") == destination.stat().st_size:
        _log(f"SUPPLEMENT HIT bytes={destination.stat().st_size:,}")
        return destination
    try:
        from pyrosm import OSM
    except ImportError as exc:
        raise RuntimeError("pyrosm >= 0.13.1 is required for the OSM address/coastline supplement") from exc
    filters: dict[str, object] = {}
    if addresses: filters["addr:housenumber"] = True
    if coastline: filters["natural"] = ["coastline"]
    destination.parent.mkdir(parents=True, exist_ok=True)
    temp = destination.with_suffix(destination.suffix + f".tmp-{os.getpid()}-{time.time_ns()}")
    _log(f"SUPPLEMENT START addresses={addresses} coastline={coastline}")
    started = time.monotonic()
    osm = OSM(str(pbf))
    frame = osm.get_data_by_custom_criteria(custom_filter=filters, filter_type="keep")
    # The pyrosm result stays opaque until extraction is complete; only pyrosm
    # serializes it here. BRUR reads the resulting file in phase 2 below.
    osm.write_pbf(frame, str(temp), subset_only=True)
    temp.replace(destination)
    _atomic_json(manifest_path, {
        "format": STAGE_FORMAT, "version": STAGE_VERSION, "complete": True,
        "source": source_identity, "predicates": predicates,
...
def _gpkg_wkb(blob: bytes | memoryview) -> bytes:
    raw = bytes(blob)
    ...
    if envelope_bytes is None:
        raise ValueError(f"unsupported GeoPackage envelope code: {envelope}")
...
def _geometry(blob):
    return from_wkb(_gpkg_wkb(blob))
...
def _truth(value) -> bool:
    return str(value or "").strip().upper() in {"T", "TRUE", "YES", "1"}


def _road_tags(row: sqlite3.Row) -> dict[str, str]:
    keys = set(row.keys())
    fclass = str(row["fclass"] or "").strip()
    tags: dict[str, str] = {}
    if fclass.startswith("track_grade"):
        tags["highway"] = "track"; tags["tracktype"] = fclass.removeprefix("track_")
    elif fclass:
        tags["highway"] = fclass
    ...
    if "oneway" in keys:
        oneway = str(row["oneway"] or "").strip().upper()
        if oneway == "F": tags["oneway"] = "yes"
        elif oneway == "T": tags["oneway"] = "-1"
        elif oneway == "B": tags["oneway"] = "no"
...
def _separation(tags: dict[str, str]) -> str:
    if tags.get("bridge") == "yes" or tags.get("tunnel") == "yes" or tags.get("layer"):
        return f"l={tags.get('layer','0')};b={tags.get('bridge','')};t={tags.get('tunnel','')}"
    return ""
...
def _traffic_signal_points(path: Path) -> tuple[list[dict], dict[tuple[int,int], list[int]]]:
    ...
    try:
        if not _table_exists(connection, "gis_osm_traffic_free"):
        ...
        for row in connection.execute("SELECT * FROM gis_osm_traffic_free WHERE fclass='traffic_signals'"):
            ...
            signal_id = int(row["osm_id"]); lon = float(geometry.x); lat = float(geometry.y)
            index = len(signals); signals.append({"osm_id": signal_id, "lon": lon, "lat": lat, "name": str(row["name"] or "")})
...
def _normalize_highways(roads_stage: Path, destination: Path, signal_stage: Path | None = None) -> tuple[dict, list[dict]]:
    signals: list[dict] = []; signal_by_coord: dict[tuple[int,int], list[int]] = {}
    if signal_stage is not None:
        signals, signal_by_coord = _traffic_signal_points(signal_stage)
        for signal in signals: signal["way_ids"] = set()
    writer = HighwayFactWriter(destination)
    connection = sqlite3.connect(roads_stage); connection.row_factory = sqlite3.Row
    ways = 0
    try:
        for row in connection.execute("SELECT * FROM gis_osm_roads_free"):
            way_id = int(row["osm_id"]); tags = _road_tags(row); geometry = _geometry(row["geom"])
            for line in _iter_lines(geometry):
                coordinates = [(float(lon), float(lat)) for lon, lat, *_ in line.coords]
                if len(coordinates) < 2: continue
                sep = _separation(tags); last = len(coordinates)-1
                node_ids: list[int] = []
                for index, (lon, lat) in enumerate(coordinates):
                    interior_sep = sep if sep and index not in {0, last} else ""
                    node_ids.append(_synthetic_node_id(lon, lat, interior_sep))
                    for signal_index in signal_by_coord.get(_qcoord(lon, lat), ()):
                        signals[signal_index]["way_ids"].add(way_id)
                writer.write_way(way_id, node_ids, coordinates, tags); ways += 1
...
def _normalize_signals(signals: list[dict], destination: Path) -> dict:
    ...
    try:
        for signal in signals:
            ...
            writer.write({
                "osm_id": int(signal["osm_id"]), "lon": float(signal["lon"]), "lat": float(signal["lat"]),
                "tags": tags, "way_ids": sorted(int(value) for value in signal.get("way_ids", ())),
            })
...
def _poi_tags(fclass: str, name: str) -> dict[str, str]:
    tags: dict[str, str] = {"geofabrik:fclass": fclass}
    rule = POI_RULES.get(fclass)
...
def _normalize_pois(stage: Path, destination: Path) -> dict:
    writer = FactWriter(destination, SCHEMAS["pois"])
    connection = sqlite3.connect(stage); connection.row_factory = sqlite3.Row
    records = 0
    try:
        for table in DOMAIN_TABLES["pois"]:
            ...
            for row in connection.execute(f"SELECT * FROM {_quote(table)}"):
                fclass = str(row["fclass"] or "") if "fclass" in row.keys() else ""
                name = str(row["name"] or "") if "name" in row.keys() else ""
                geometry = _geometry(row["geom"]); osm_id = int(row["osm_id"])
                tags = _poi_tags(fclass, name)
                if "places_" in table or table.endswith("places_free"):
                    tags.pop("amenity", None); tags["place"] = fclass
                if geometry.geom_type == "Point":
                    x, y = project(float(geometry.x), float(geometry.y))
                    writer.write({"osm_type":"node", "osm_id":osm_id, "x":x, "y":y, "tags":tags}); records += 1
                else:
                    polygons = list(_iter_polygons(geometry))
                    if not polygons: continue
                    centroid = geometry.centroid; x, y = project(float(centroid.x), float(centroid.y))
                    projected_geometry: list[list[list[float]]] = []
                    for polygon in polygons:
                        projected_geometry.append([[*project(float(lon), float(lat))] for lon, lat, *_ in polygon.exterior.coords])
                    writer.write({"osm_type":"way", "osm_id":osm_id, "x":x, "y":y, "geometry":projected_geometry, "tags":tags}); records += 1
...
def _polygon_fact_geometry(geometry) -> list[dict]:
    ...
    for polygon in _iter_polygons(geometry):
        outer = [[*project(float(lon), float(lat))] for lon, lat, *_ in list(polygon.exterior.coords)[:-1]]
        holes = [
            [[*project(float(lon), float(lat))] for lon, lat, *_ in list(ring.coords)[:-1]]
...
def _area_tags(table: str, row: sqlite3.Row) -> dict[str, str] | None:
    ...
    if table == "gis_osm_buildings_a_free":
        building_type = str(row["type"] or "") if "type" in row.keys() else ""
        tags["building"] = building_type or "yes"
        for column, key in (("height", "height"), ("levels", "building:levels"), ("building_levels", "building:levels")):
            if column in row.keys() and row[column] not in (None, ""): tags[key] = str(row[column])
        ...
        code = int(row["code"] or 0) if "code" in row.keys() else 0
...
class _SupplementHandler(osmium.SimpleHandler):
    ...
    def _address(self, osm_type: str, osm_id: int, tags, x: float, y: float) -> None:
        ...
        values = {key:str(tags.get(key)) for key in ADDRESS_TAGS if tags.get(key) is not None}
        self.address_writer.write({"osm_type":osm_type, "osm_id":osm_id, "x":x, "y":y, "tags":values})
    ...
    def way(self, way) -> None:
        ...
        try:
            ...
            return
        if self.area_writer is not None and way.tags.get("natural") == "coastline" and len(points) >= 2:
            self.area_writer.write_payload(_encode_coastline(int(way.id), [project(lon,lat) for lon,lat in points]))
        if self.address_writer is not None and points:
            projected = [project(lon,lat) for lon,lat in points]; x = sum(p[0] for p in projected)/len(projected); y = sum(p[1] for p in projected)/len(projected)
            self._address("way", int(way.id), way.tags, x, y)


def _normalize_areas(stage: Path, supplement: Path, destination: Path) -> dict:
    writer = _AreaWriter(destination); connection = sqlite3.connect(stage); connection.row_factory = sqlite3.Row
    count = 0
    try:
        for table in DOMAIN_TABLES["areas"]:
            ...
            for row in connection.execute(f"SELECT * FROM {_quote(table)}"):
                ...
                if not polygons: continue
                osm_id = int(row["osm_id"]); writer.write_payload(_encode_area(RECORD_AREA_WAY, osm_id, osm_id, tags, polygons)); count += 1
                if count % PROGRESS_INTERVAL == 0: _log(f"NORMALIZE areas={count:,}")
            _log(f"NORMALIZE areas layer={table} records={count:,}")
        _SupplementHandler(None, writer).apply_file(str(supplement), locations=True)
        return writer.publish()
    except BaseException:
        writer.abort(); raise
    finally:
...
def _normalize_addresses(supplement: Path, destination: Path) -> dict:
    ...
    try:
        _SupplementHandler(writer, None).apply_file(str(supplement), locations=True)
        return writer.publish()
    except BaseException:
        writer.abort(); raise
...
def _entry(route: str, report: dict, path: Path, source_identity: dict, elapsed: float) -> dict:
    return {
        "version": ROUTE_VERSIONS[route], "extractor_version": EXTRACTOR_VERSION,
        "extractor": "geofabrik-gpkg+pyrosm-supplement", "complete": True,
        "file": _route_filename(route), "records": int(report.get("records", 0)),
        "size_bytes": int(path.stat().st_size), "sha256": str(report["sha256"]),
        "source_identity": source_identity, "artifact_metadata": _artifact_metadata(path),
        "elapsed_seconds": round(elapsed, 3),
    }


def _composite_identity(gpkg_identity: dict, pbf_identity: dict | None) -> dict:
    payload = {"geofabrik": gpkg_identity, "osm_supplement": pbf_identity}
    digest = hashlib.sha256(json.dumps(payload, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
    return {"algorithm":"sha256-composite", "digest":digest, "sources":payload}
...
def build_geofabrik_source_caches(gpkg: Path, address_pbf: Path | None, cache_dir: Path, routes: Iterable[str] = ALL_ROUTES) -> dict[str, Path]:
    ...
    if need_supplement and (address_pbf is None or not address_pbf.is_file()):
        raise ValueError("--address-pbf is required for addresses/coastline when building from GeoPackage")
    cache_dir.mkdir(parents=True, exist_ok=True); stage_dir = cache_dir / STAGE_DIR_NAME
    gpkg_identity = compute_source_identity(gpkg, cache_dir / "geofabrik_identity.json")
    pbf_identity = compute_source_identity(address_pbf, cache_dir / "supplement_identity.json") if need_supplement and address_pbf is not None else None
    route_sources = {
        route: (_composite_identity(gpkg_identity, pbf_identity) if route == "areas" else pbf_identity if route == "addresses" else gpkg_identity)
        for route in requested
    }
    manifest_path = cache_dir / "manifest.json"; old = _load_json(manifest_path); old_routes = old.get("routes", {}) if isinstance(old.get("routes"), dict) else {}
    entries: dict[str, dict] = {route:dict(entry) for route,entry in old_routes.items() if route in ROUTE_VERSIONS and isinstance(entry,dict)}
    stale = {route for route in requested if not _route_valid(cache_dir, route, entries.get(route), route_sources[route])}
    run = {"format":CACHE_FORMAT, "status":"RUNNING", "started_at":_now(), "requested":list(requested), "blocks":{}}
    _atomic_json(cache_dir / "last_run.json", run)
    for route in requested:
        run["blocks"][route] = {"status":"REBUILD" if route in stale else "CACHE HIT"}
        _log(f"PLAN route={route} status={run['blocks'][route]['status']}")
    if not stale:
        run.update({"status":"DONE", "completed_at":_now(), "source":_composite_identity(gpkg_identity,pbf_identity)})
        _atomic_json(cache_dir / "last_run.json", run)
        return {route:cache_dir/_route_filename(route) for route in requested}

    # Phase 1: every provider extraction is completed and published first.
    staged = stage_geopackage(gpkg, stage_dir, stale, gpkg_identity)
    supplement: Path | None = None
    if stale & {"addresses", "areas"}:
        assert address_pbf is not None and pbf_identity is not None
        supplement = stage_pyrosm_supplement(
            address_pbf, stage_dir / "osm_supplement.osm.pbf",
            addresses="addresses" in stale, coastline="areas" in stale, source_identity=pbf_identity,
        )

    # Phase 2: BRUR normalization starts only after all requested stage files exist.
    signal_records: list[dict] | None = None
    if stale & {"highways", "traffic_signals"}:
        started = time.monotonic(); highways_report, signal_records = _normalize_highways(staged["roads"], cache_dir/"highways.brfacts", staged.get("traffic"))
        elapsed = time.monotonic()-started
        if "highways" in stale:
            entries["highways"] = _entry("highways", highways_report, cache_dir/"highways.brfacts", route_sources["highways"], elapsed)
            run["blocks"]["highways"] = {"status":"DONE", **entries["highways"]}
        elif (cache_dir/"highways.brfacts").exists():
            # A traffic-only rebuild may recompute the shared normalized road scan,
            # but does not change the highway route fingerprint/source identity.
            pass
    if "traffic_signals" in stale:
        if signal_records is None:
            signal_records, _ = _traffic_signal_points(staged["traffic"]); [item.__setitem__("way_ids", set()) for item in signal_records]
        started=time.monotonic(); report=_normalize_signals(signal_records, cache_dir/"traffic_signals.brfacts"); elapsed=time.monotonic()-started
        entries["traffic_signals"]=_entry("traffic_signals",report,cache_dir/"traffic_signals.brfacts",route_sources["traffic_signals"],elapsed); run["blocks"]["traffic_signals"]={"status":"DONE",**entries["traffic_signals"]}
    if "pois" in stale:
        started=time.monotonic(); report=_normalize_pois(staged["pois"],cache_dir/"pois.brfacts"); elapsed=time.monotonic()-started
        entries["pois"]=_entry("pois",report,cache_dir/"pois.brfacts",route_sources["pois"],elapsed); run["blocks"]["pois"]={"status":"DONE",**entries["pois"]}
    if "addresses" in stale:
        assert supplement is not None
        started=time.monotonic(); report=_normalize_addresses(supplement,cache_dir/"addresses.brfacts"); elapsed=time.monotonic()-started
        entries["addresses"]=_entry("addresses",report,cache_dir/"addresses.brfacts",route_sources["addresses"],elapsed); run["blocks"]["addresses"]={"status":"DONE",**entries["addresses"]}
    if "areas" in stale:
        assert supplement is not None
        started=time.monotonic(); report=_normalize_areas(staged["areas"],supplement,cache_dir/"areas.baf"); elapsed=time.monotonic()-started
        entries["areas"]=_entry("areas",report,cache_dir/"areas.baf",route_sources["areas"],elapsed); run["blocks"]["areas"]={"status":"DONE",**entries["areas"]}

    composite = _composite_identity(gpkg_identity, pbf_identity)
    _atomic_json(manifest_path, {"format":CACHE_FORMAT, "source":composite, "routes":entries})
    run.update({"status":"DONE", "completed_at":_now(), "source":composite})
    _atomic_json(cache_dir/"last_run.json",run)
    return {route:cache_dir/_route_filename(route) for route in requested}
