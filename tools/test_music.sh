...
# Runs the reusable music subsystem's deterministic Godot correctness contracts.
...
SCRIPT="tests/godot/test_music_playback_state.gd"
MARKER="godot music playback-state tests: OK"
...
if ! grep -Fq "$MARKER" <<<"$output"; then
  echo "music Godot tests did not reach success marker: $MARKER" >&2
  exit 1
fi

echo "music automated tests: OK"
