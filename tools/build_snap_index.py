#!/usr/bin/env python3
"""Build the persistent BRS1 road-snap index from an existing BRG1 graph.

Dependencies:
- routing_graph_view.py memory-maps BRG1 without expanding Sweden in memory.
- gps_snap_index.py owns the compact BRS1 format and builder.
"""

from __future__ import annotations

import argparse
from pathlib import Path

from gps_snap_index import DEFAULT_CELL_SIZE_M, build_snap_index
from routing_graph_view import RoutingGraphView


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("graph", type=Path, nargs="?", default=Path("world_data/routing.brg"))
    parser.add_argument("--output", type=Path, default=Path("world_data/routing_snap.brs"))
    parser.add_argument("--cell-size", type=float, default=DEFAULT_CELL_SIZE_M)
    args = parser.parse_args()

    print(f"[snap-index] source: {args.graph}", flush=True)
    with RoutingGraphView(args.graph) as graph:
        print(
            f"[snap-index] graph: {len(graph.nodes):,} nodes, {len(graph.edges):,} directed edges",
            flush=True,
        )
        build_snap_index(graph, args.output, args.cell_size)


if __name__ == "__main__":
    main()
