...
from build_routing_facts import build_routing_facts
from gps_snap_index import build_snap_index
from routing_graph_view import RoutingGraphView

DATASET_FORMAT = "BRG1+BRS2+BRH1"
DATASET_PAYLOAD_FILES = ("routing.brg", "routing_snap.brs", "routing_geometry.brh")
...
def build_routing_dataset(source: Path, output: Path) -> dict:
    ...
    try:
        report = build_routing_facts(source, staging) if source.suffix == ".brfacts" else build_routing(source, staging)
        graph_path = staging / "routing.brg"; snap_path = staging / "routing_snap.brs"
        with RoutingGraphView(graph_path) as graph:
            snap_report = build_snap_index(graph, snap_path)
        for name in DATASET_PAYLOAD_FILES:
            if not (staging / name).is_file(): raise RuntimeError(f"routing dataset build did not produce {name}")
        report = dict(report)
        report["routing_dataset_format"] = DATASET_FORMAT
        report["routing_dataset_sha256"] = {name:_sha256(staging/name) for name in DATASET_PAYLOAD_FILES}
        ...
        (staging / "routing_stats.json").write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
        for name in DATASET_FILES:
            if not (staging / name).is_file(): raise RuntimeError(f"routing dataset build did not produce {name}")
        for name in DATASET_FILES: (staging/name).replace(output/name)
        return report
    finally:
        if staging.exists(): shutil.rmtree(staging)
...
def main() -> None:
    parser=argparse.ArgumentParser(); parser.add_argument("source",type=Path); parser.add_argument("--output",type=Path,default=Path("world_data")); args=parser.parse_args(); build_routing_dataset(args.source,args.output)
