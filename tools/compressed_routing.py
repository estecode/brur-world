...
class CompressedGraphAccumulator:
    ...
    def close(self) -> None:
        """Release the temporary geometry spool as soon as publication is complete."""
        if not self._geometry_spool.closed:
            self._geometry_spool.close()
    ...
    def add_way(self, way: WayInput) -> None:
        ...
        for index in range(1, len(way.node_ids)):
            ...
            if accumulated_length > 0.0:
                ...
                self._append_edge(
                    way, start_index, source_id, target_id, accumulated_length, forward_speed,
                    access.access_class, access.reason,
                    flags | (FLAG_AGAINST_ONEWAY if oneway == -1 else 0), layer,
                    geometry_offset, geometry_count, False,
                )
                self._append_edge(
                    way, start_index, target_id, source_id, accumulated_length, backward_speed,
                    access.access_class, access.reason,
                    flags | (FLAG_AGAINST_ONEWAY if oneway == 1 else 0), layer,
                    geometry_offset, geometry_count, True,
                )
            start_index = index
            accumulated_length = 0.0
    ...
    def _append_edge(
        ...
        self._edges.append(
            _PendingEdge(
                way_id=int(way.way_id), segment_index=int(segment_index), source_osm=source_osm,
                target_osm=target_osm, length_m=length_m, speed_kmh=speed.kmh,
                road_class=ROAD_CLASS[highway], access_class=int(access_class), flags=flags,
                speed_source=int(speed.source), layer=layer, access_reason=int(access_reason),
                geometry_offset=geometry_offset, geometry_count=geometry_count,
    ...
    def finish(self, progress: ProgressCallback | None = None) -> RoutingGraph:
        ...
        for processed, pending in enumerate(grouped, start=1):
            ...
            edges.append(
                GraphEdge(
                    pending.way_id, pending.segment_index, node_index[pending.source_osm],
                    node_index[pending.target_osm], pending.length_m, pending.speed_kmh,
                    pending.road_class, AccessClass(pending.access_class), pending.flags,
                    SpeedSource(pending.speed_source), pending.layer, pending.access_reason,
                )
            )
    ...
    def write_route_geometry(self, path: Path, progress: ProgressCallback | None = None) -> None:
        ...
        with path.open("wb") as handle:
            handle.write(GEOMETRY_HEADER.pack(GEOMETRY_MAGIC, len(self._grouped_geometry), self._geometry_point_count))
...
def write_brg1_with_progress(
    ...
    with path.open("wb") as handle:
        ...
        for processed, node in enumerate(graph.nodes, start=1):
            handle.write(NODE_RECORD.pack(node.osm_id, node.lon, node.lat, node.x, node.y, node.adjacency_offset, node.adjacency_count))
            if progress and processed % 100_000 == 0:
                progress("writing nodes", processed, f"{len(graph.nodes):,} nodes total")
        for processed, edge in enumerate(graph.edges, start=1):
            handle.write(EDGE_RECORD.pack(
                edge.way_id, edge.segment_index, edge.source_index, edge.target_index,
                edge.length_m, edge.speed_kmh, edge.road_class, int(edge.access_class),
                edge.flags, int(edge.speed_source), edge.layer, int(edge.access_reason),
            ))
            if progress and processed % 100_000 == 0:
